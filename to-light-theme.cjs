...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      content = content.replace(/bg-\[#251210\]\/50/g, 'bg-slate-50/50');
      content = content.replace(/bg-\[#251210\]/g, 'bg-slate-50');
      content = content.replace(/bg-\[#2d1816\]\/50/g, 'bg-slate-100/50');
      content = content.replace(/bg-\[#2d1816\]/g, 'bg-slate-100');
      
      content = content.replace(/bg-\[#000000\]/g, 'bg-[#faf8f5]'); // for main page wrapper?
      ...
      content = content.replace(/text-\[#a28c89\]\/10/g, 'text-slate-500/10');
      content = content.replace(/text-\[#a28c89\]\/20/g, 'text-slate-500/20');
      ...
      content = content.replace(/group-hover:text-\[#a28c89\]/g, 'group-hover:text-slate-500');
...
// Ensure the path assumes we are executing from the right place
